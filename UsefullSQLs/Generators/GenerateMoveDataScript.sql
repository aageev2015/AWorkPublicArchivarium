...
'
from GoldentelecomDemo330..sysobjects t1
inner join syscolumns t2 on (
	t1.id=t2.id
)
left join streamDemo330..sysobjects t3 on (
	t1.name=t3.name
)
left join Streamdemo330..syscolumns t4 on (
	t3.id=t4.id
	and t4.name=t2.name
)
...
from tbl_Account t1, StreamDemo330..tbl_Account t2
where t1.id=t2.id'
