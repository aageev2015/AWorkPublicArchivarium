set nocount on
declare @TableName nvarchar(250) = 'tbl_xcl_ContactCommunication'
...
declare @tColList nvarchar(4000) = ''
declare @tValList nvarchar(4000) = ''
...
+ '+@Q1+', '+@Q1+' + ' end) + 
	'isnull(' + ( 
		case 
		when Data_type in ('uniqueidentifier', 'datetime', 'time', 'smalldatetime' ) 
		...
		else ('cast(' + COLUMN_NAME + ' as nvarchar(' + isnull(cast(Character_Maximum_length as nvarchar(20)), 100) + '))')
		 end )+
	', ' + @Q1+'null'+@Q1+')'
