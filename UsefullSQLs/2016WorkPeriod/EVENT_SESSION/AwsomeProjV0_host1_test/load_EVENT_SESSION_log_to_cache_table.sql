/*
select [XEvent] from tbl_EVENT_SESSION_log
order by [timestamp] desc, [event_sequence] desc
*/
...
FROM (
    ...
    FROM sys.fn_xe_file_target_read_file(N'c:\SQLLogs\AwesomeSystemV0_session*.xel', --FILENAME SHOULD BE CHECKED
	N'c:\SQLLogs\AwesomeSystemV0_session*.xem', null, null)) as src				   --FILENAME SHOULD BE CHECKED
ORDER BY 
    DATEADD(hh, 
            DATEDIFF(hh, GETUTCDATE(), CURRENT_TIMESTAMP), 
