SELECT 
	...
	XEvent.value('(event/action[@name="context_info"]/value)[1]', 'nvarchar(1000)') AS [context_info],
...
FROM (
    ...
    FROM sys.fn_xe_file_target_read_file(N'c:\SQLLogs\AwsomeProjV0_session*.xel', --FILENAME SHOULD BE CHECKED
	N'c:\SQLLogs\AwsomeProjV0_session*.xem', null, null)) as src				   --FILENAME SHOULD BE CHECKED
ORDER BY 
    DATEADD(hh, 
            DATEDIFF(hh, GETUTCDATE(), CURRENT_TIMESTAMP), 
