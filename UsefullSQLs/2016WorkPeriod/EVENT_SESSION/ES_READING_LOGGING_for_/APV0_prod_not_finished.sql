...
FROM (
    ...
    FROM sys.fn_xe_file_target_read_file('<full file path here>*.xel', --FILENAME SHOULD BE CHECKED
	'<full file path here>*.xem', null, null)) as src				   --FILENAME SHOULD BE CHECKED
ORDER BY 
    DATEADD(hh, 
            DATEDIFF(hh, GETUTCDATE(), CURRENT_TIMESTAMP), 
